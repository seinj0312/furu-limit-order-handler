...
export const getTokenFromFaucet = async (
  ...
  // Fetch actual Faucet
  const faucet = getFacetByToken(tokenAddress.toLowerCase());
  ...
  if (
    faucet !== getFacetByToken("0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee") &&
    faucetEthBalance.lt(oneEth)
  ) {
    // Fund faucet account with ETH
    const ethFaucet = getFacetByToken(
...
const getFacetByToken = (tokenAddress: string): string => {
  switch (tokenAddress) {
